...
function resolveGatewayPort(cfg: ANIMAPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.ANIMA_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.ANIMABOT_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: ANIMAPluginApi["config"]): ResolveAuthResult {
  ...
  const token =
    pickFirstDefined([
      ...
      process.env.ANIMABOT_GATEWAY_TOKEN,
      cfg.gateway?.auth?.token,
    ]) ?? undefined;
  const password =
    pickFirstDefined([
      process.env.ANIMA_GATEWAY_PASSWORD,
      process.env.ANIMABOT_GATEWAY_PASSWORD,
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the ANIMA iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the ANIMA iOS app:",
            "",
            "```",
            qrAscii,
            "```",
