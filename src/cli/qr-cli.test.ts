...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("ANIMA_GATEWAY_TOKEN", "");
    vi.stubEnv("ANIMABOT_GATEWAY_TOKEN", "");
    vi.stubEnv("ANIMA_GATEWAY_PASSWORD", "");
    vi.stubEnv("ANIMABOT_GATEWAY_PASSWORD", "");
  });
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("anima devices approve <requestId>");
  });
