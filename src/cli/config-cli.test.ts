...
const mockWriteConfigFile = vi.fn<
  (cfg: ANIMAConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: ANIMAConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: ANIMAConfig, config: ANIMAConfig) {
...
describe("config cli", () => {
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: ANIMAConfig = { gateway: { port: 18789 } };
