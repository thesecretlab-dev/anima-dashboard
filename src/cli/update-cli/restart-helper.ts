...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  ...
  return `${resolveGatewaySystemdServiceName(env.ANIMA_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = env.ANIMA_LAUNCHD_LABEL?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.ANIMA_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `anima-restart-${timestamp}.sh`;
...
`;
      ...
      filename = `anima-restart-${timestamp}.bat`;
