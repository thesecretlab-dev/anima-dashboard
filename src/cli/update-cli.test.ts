...
import type { ANIMAConfig, ConfigFileSnapshot } from "../config/types.anima.js";
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/anima-update-tests";
  let fixtureCount = 0;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveANIMAPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("anima-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    prepareRestartScript.mockResolvedValue("/tmp/anima-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "anima-gateway" }],
      hints: [],
    });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as ANIMAConfig,
        });
      },
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("anima-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ ANIMA_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        ...
        expect(process.env.ANIMA_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("anima-update-wizard");
    await withEnvAsync({ ANIMA_GIT_DIR: tempDir }, async () => {
      setTty(true);
