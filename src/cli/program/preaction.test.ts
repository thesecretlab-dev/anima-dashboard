...
beforeEach(() => {
  ...
  originalHideBanner = process.env.ANIMA_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.ANIMA_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.ANIMA_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("anima");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "anima", "status", "--debug"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "anima", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "anima", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "anima", "--version"],
    });
  ...
  it("hides banner when ANIMA_HIDE_BANNER is truthy", async () => {
    process.env.ANIMA_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "anima", "status"],
    });
