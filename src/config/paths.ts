...
const LEGACY_STATE_DIRNAMES = [".animabot", ".moldbot", ".animabot"] as const;
const NEW_STATE_DIRNAME = ".anima";
const CONFIG_FILENAME = "anima.json";
const LEGACY_CONFIG_FILENAMES = ["animabot.json", "moldbot.json", "animabot.json"] as const;
...
export function resolveStateDir(
  ...
  const override = env.ANIMA_STATE_DIR?.trim() || env.ANIMABOT_STATE_DIR?.trim();
  if (override) {
...
export function resolveCanonicalConfigPath(
  ...
  const override = env.ANIMA_CONFIG_PATH?.trim() || env.ANIMABOT_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.ANIMA_CONFIG_PATH?.trim() || env.ANIMABOT_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const animaStateDir = env.ANIMA_STATE_DIR?.trim() || env.ANIMABOT_STATE_DIR?.trim();
  if (animaStateDir) {
...
export function resolveGatewayPort(
  ...
  const envRaw = env.ANIMA_GATEWAY_PORT?.trim() || env.ANIMABOT_GATEWAY_PORT?.trim();
  if (envRaw) {
