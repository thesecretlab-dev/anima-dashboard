...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("anima-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "anima-sandbox-browser-global" },
      agentBrowser: { network: "anima-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("anima-sandbox-browser-agent");
  });
