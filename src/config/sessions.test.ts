...
describe("sessions", () => {
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ANIMA_STATE_DIR: stateDir }, fn);
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.anima");
  ...
  it("resolves cross-agent paths when ANIMA_STATE_DIR differs from stored paths", () => {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.anima"), () => {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
