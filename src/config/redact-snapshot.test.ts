...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/anima/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/anima/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/anima/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/anima/nickserv-password.txt");
