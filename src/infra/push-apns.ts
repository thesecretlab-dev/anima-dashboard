...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ANIMA_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ANIMA_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ANIMA_APNS_TEAM_ID and ANIMA_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ANIMA_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ANIMA_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ANIMA_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ANIMA_APNS_PRIVATE_KEY_P8 or ANIMA_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ANIMA_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createANIMAPushMetadata(params: {
