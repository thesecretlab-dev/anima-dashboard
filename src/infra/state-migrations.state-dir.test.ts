...
describe("legacy state dir auto-migration", () => {
  it("follows legacy symlink when it points at another legacy dir (animabot -> animabot)", async () => {
    const root = await makeTempRoot();
    const legacySymlink = path.join(root, ".animabot");
    ...
    expect(fs.readFileSync(path.join(root, ".animabot", "marker.txt"), "utf-8")).toBe("ok");
    expect(fs.readFileSync(path.join(root, ".animabot", "marker.txt"), "utf-8")).toBe("ok");
  });
});
