...
vi.mock("./anima-root.js", () => ({
  resolveANIMAPackageRoot: vi.fn(async () => null),
  resolveANIMAPackageRootSync: vi.fn(() => null),
}));
...
let animaRoot: typeof import("./anima-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    animaRoot = await import("./anima-root.js");
  });
  ...
  it("uses resolveANIMAPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/anima");
    (
      animaRoot.resolveANIMAPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/anima"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "anima" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "anima.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-anima");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/anima-bundle");
    (
      animaRoot.resolveANIMAPackageRootSync as unknown as ReturnType<typeof vi.fn>
