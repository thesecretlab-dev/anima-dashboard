...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__anima_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "anima-root");
...
describe("resolveANIMAPackageRoot", () => {
  let resolveANIMAPackageRoot: typeof import("./anima-root.js").resolveANIMAPackageRoot;
  let resolveANIMAPackageRootSync: typeof import("./anima-root.js").resolveANIMAPackageRootSync;

  beforeAll(async () => {
    ({ resolveANIMAPackageRoot, resolveANIMAPackageRootSync } =
      await import("./anima-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "anima");
    const pkgRoot = path.join(project, "node_modules", "anima");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "anima" }));

    expect(resolveANIMAPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "anima");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "anima.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "anima" }));

    expect(resolveANIMAPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveANIMAPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-anima package roots", async () => {
    const pkgRoot = fx("not-anima");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-anima" }));

    expect(resolveANIMAPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveANIMAPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveANIMAPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
