...
let ensureANIMACliOnPath: typeof import("./path-env.js").ensureANIMACliOnPath;

describe("ensureANIMACliOnPath", () => {
  const envKeys = [
    "PATH",
    "ANIMA_PATH_BOOTSTRAPPED",
    "ANIMA_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureANIMACliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling anima exists", () => {
    const tmp = abs("/tmp/anima-path/case-bundled");
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/anima-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/anima-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "anima");
    setDir(tmp);
    setDir(appBinDir);
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/anima-path/case-linuxbrew");
