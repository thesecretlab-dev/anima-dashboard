...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ANIMA_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*anima-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "anima";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "anima");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "anima-gw");
  ...
  records.push(`_anima-gw._tcp IN PTR ${instanceLabel}._anima-gw._tcp`);
  records.push(`${instanceLabel}._anima-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._anima-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; anima-content-hash: ${contentHash}\n${contentBody}`;
}
