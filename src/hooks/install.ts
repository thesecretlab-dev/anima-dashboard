...
async function installHookPackageFromDir(params: {
  ...
  for (const entry of hookEntries) {
    ...
    if (!isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `anima.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `anima.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromArchive(params: {
  ...
  return await withTempDir("anima-hook-", async (tmpDir) => {
