...
type EnvSnapshot = {
  HOME?: string;
  ANIMA_HOME?: string;
  ANIMA_STATE_DIR?: string;
  ANIMA_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    ANIMA_HOME: process.env.ANIMA_HOME,
    ANIMA_STATE_DIR: process.env.ANIMA_STATE_DIR,
    ANIMA_OAUTH_DIR: process.env.ANIMA_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: ANIMAConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "anima-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.ANIMA_STATE_DIR, { recursive: true, mode: 0o700 });
