...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["ANIMABOT_GATEWAY_TOKEN", await getenv("ANIMABOT_GATEWAY_TOKEN")],
    ["ANIMABOT_GATEWAY_PASSWORD", await getenv("ANIMABOT_GATEWAY_PASSWORD")],
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("ANIMABOT_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
