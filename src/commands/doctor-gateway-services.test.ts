...
async function runRepair(cfg: ANIMAConfig) {
...
function setupGatewayTokenRepairScenario(expectedToken: string) {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      ANIMA_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service ANIMA_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
  mocks.buildGatewayInstallPlan.mockResolvedValue({
    ...
    environment: {
      ANIMA_GATEWAY_TOKEN: expectedToken,
    },
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses ANIMA_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ ANIMA_GATEWAY_TOKEN: "env-token" }, async () => {
