...
function resolveGatewayAuthToken(cfg: ANIMAConfig, env: NodeJS.ProcessEnv): string | undefined {
  const configToken = cfg.gateway?.auth?.token?.trim();
  if (configToken) {
    return configToken;
  }
  const envToken = env.ANIMA_GATEWAY_TOKEN ?? env.ANIMABOT_GATEWAY_TOKEN;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({
      message: "Remove legacy gateway services (animabot/animabot) now?",
      initialValue: true,
    });
    if (shouldRemove) {
