...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under ANIMA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("anima-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-oauth-sync-"));
    ...
    process.env.ANIMA_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-oauth-nosync-"));
    process.env.ANIMA_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ANIMA_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-oauth-external-"));
    process.env.ANIMA_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ANIMA_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ANIMA_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("anima-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
