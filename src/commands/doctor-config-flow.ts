...
function scanDiscordNumericIdEntries(cfg: ANIMAConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: ANIMAConfig): {
  config: ANIMAConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: ANIMAConfig): {
  config: ANIMAConfig;
  changes: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const legacyCandidates = [
    path.join(home, ".animabot", "animabot.json"),
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("anima doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("anima doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
