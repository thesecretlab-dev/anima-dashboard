...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: ANIMAConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: ANIMAConfig) => { changed: boolean; next: ANIMAConfig };
  defaultModel: string;
  overrideConfig: ANIMAConfig;
  alreadyDefaultConfig: ANIMAConfig;
};
