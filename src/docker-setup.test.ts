...
describe("docker-setup.sh", () => {
  ...
  it("rejects injected multiline ANIMA_EXTRA_MOUNTS values", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    expect(result.stderr).toContain("ANIMA_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid ANIMA_EXTRA_MOUNTS mount format", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        ANIMA_EXTRA_MOUNTS: "bad mount spec",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
  ...
  it("rejects invalid ANIMA_HOME_VOLUME names", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ANIMA_HOME_VOLUME must match");
  });
