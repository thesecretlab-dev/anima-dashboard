...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "anima", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "anima", version: "2.3.4" });
  ...
  it("ignores non-anima package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers ANIMA_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ANIMA_VERSION: "9.9.9",
        ANIMA_SERVICE_VERSION: "2.2.2",
