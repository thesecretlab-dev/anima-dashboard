...
function resolveVoiceTtsConfig(params: { cfg: ANIMAConfig; override?: TtsConfig }): {
  cfg: ANIMAConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredANIMATmpDir(), "discord-voice-"));
