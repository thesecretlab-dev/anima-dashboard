...
const resolveStorePath = vi.fn(() => "/tmp/anima-discord-process-test-sessions.json");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/anima-discord-process-test-sessions.json");
  ...
  it("streams block previews using draft chunking", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/anima-discord-process-test-sessions.json" },
        channels: {
          discord: {
