  ...
  const token =
    ...
    (!context.urlOverride
      ? context.isRemoteMode
        ...
        : trimToUndefined(process.env.ANIMA_GATEWAY_TOKEN) ||
          trimToUndefined(process.env.ANIMABOT_GATEWAY_TOKEN) ||
          trimToUndefined(authToken)
      : undefined);
  const password =
    context.explicitAuth.password ||
    (!context.urlOverride
      ? trimToUndefined(process.env.ANIMA_GATEWAY_PASSWORD) ||
        trimToUndefined(process.env.ANIMABOT_GATEWAY_PASSWORD) ||
        (context.isRemoteMode
