...
const { createANIMATools } = await import("../agents/anima-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ANIMA_GATEWAY_PORT", "ANIMA_GATEWAY_TOKEN"]);
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("ANIMA_CONFIG_PATH missing in gateway test environment");
    }
