...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): ANIMAConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as ANIMAConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): ANIMAConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: ANIMAConfig, accountId = "media") {
    await handleTelegramAction(
