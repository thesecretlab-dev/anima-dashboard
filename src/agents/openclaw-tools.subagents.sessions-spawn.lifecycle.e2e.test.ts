...
} from "./anima-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateANIMATools = (typeof import("./anima-tools.js"))["createANIMATools"];
type CreateANIMAToolsOpts = Parameters<CreateANIMATools>[0];

async function getSessionsSpawnTool(opts: CreateANIMAToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createANIMATools } = await import("./anima-tools.js");
  const tool = createANIMATools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
