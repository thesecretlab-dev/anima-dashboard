...
function createSandbox(overrides?: Partial<SandboxContext>): SandboxContext {
  return createSandboxTestContext({
    overrides: {
      containerName: "animabot-sbx-test",
      ...overrides,
    },
    dockerOverrides: {
      image: "animabot-sandbox:bookworm-slim",
      containerPrefix: "animabot-sbx-",
    },
  });
}
...
describe("sandbox fs bridge shell compatibility", () => {
  ...
  it("resolves bind-mounted absolute container paths for reads", async () => {
    ...
    expect(args).toEqual(
      expect.arrayContaining(["animabot-sbx-test", "sh", "-c", 'set -eu; cat -- "$1"']),
    );
