...
import { createANIMAReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createANIMACodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createANIMACodingTools({ senderIsOwner: true }).find(
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    ...
    const googleTools = createANIMACodingTools({
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createANIMAReadTool(
      baseRead as unknown as Parameters<typeof createANIMAReadTool>[0],
    );
