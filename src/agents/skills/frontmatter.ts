...
import {
  ...
  normalizeStringList,
  parseANIMAManifestInstallBase,
  parseFrontmatterBool,
  resolveANIMAManifestBlock,
  resolveANIMAManifestInstall,
  resolveANIMAManifestOs,
  resolveANIMAManifestRequires,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseANIMAManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveANIMAMetadata(
  ...
  const requires = resolveANIMAManifestRequires(metadataObj);
  const install = resolveANIMAManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveANIMAManifestOs(metadataObj);
  return {
