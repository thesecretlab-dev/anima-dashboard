...
function resolveSkillsLimits(config?: ANIMAConfig): ResolvedSkillsLimits {
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: ANIMAConfig }): {
...
export function buildWorkspaceSkillSnapshot(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`anima skills check\` to audit.`
    : "";
