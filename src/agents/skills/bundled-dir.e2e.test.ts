...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ANIMA_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns ANIMA_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "anima-bundled-override-"));
    process.env.ANIMA_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
