...
function resolvePayloadLogConfig(env: NodeJS.ProcessEnv): PayloadLogConfig {
  const requested = parseBooleanValue(env.ANIMA_ANTHROPIC_PAYLOAD_LOG) ?? false;
  const isDevelopment = String(env.NODE_ENV ?? "").trim() === "development";
  const allowInProduction = parseBooleanValue(env.ANIMA_ALLOW_PAYLOAD_LOG) ?? false;
  const enabled = requested && (isDevelopment || allowInProduction);
...
function getWriter(filePath: string): PayloadLogWriter {
  return getQueuedFileWriter(writers, filePath, { dirMode: 0o700, fileMode: 0o600 });
}
...
export function createAnthropicPayloadLogger(params: {
  ...
  const recordUsage: AnthropicPayloadLogger["recordUsage"] = (messages, error) => {
    ...
    record({
      ...
      ts: new Date().toISOString(),
      stage: "usage",
      usage: redactPayload(usage) as Record<string, unknown>,
      error: errorMessage,
    });
