...
const GOOGLE_LIVE_SINGLE_KEY = "ANIMA_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ANIMA_LIVE_ANTHROPIC_KEY",
    listVar: "ANIMA_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ANIMA_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ANIMA_LIVE_${base}_KEY`;
