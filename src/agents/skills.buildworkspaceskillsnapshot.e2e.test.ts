...
describe("buildWorkspaceSkillSnapshot", () => {
  it("returns an empty snapshot when skills dirs are missing", async () => {
    const workspaceDir = await createTempDir("anima-");
  ...
  it("limits discovery for nested repo-style skills roots (dir/skills/*)", async () => {
    const workspaceDir = await createTempDir("anima-");
    const repoDir = await createTempDir("anima-skills-repo-");
  ...
  it("enforces maxSkillFileBytes for root-level SKILL.md", async () => {
    const workspaceDir = await createTempDir("anima-");
    const rootSkillDir = await createTempDir("anima-root-skill-");
