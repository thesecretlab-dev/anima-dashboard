...
describe("resolveGatewayConnection", () => {
  ...
  it("uses ANIMA_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ ANIMA_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers ANIMA_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ ANIMA_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
