...
function resolveGatewayPort(cfg: ANIMAConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: ANIMAConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    env.ANIMA_GATEWAY_TOKEN?.trim() ||
    env.ANIMABOT_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    env.ANIMA_GATEWAY_PASSWORD?.trim() ||
    env.ANIMABOT_GATEWAY_PASSWORD?.trim() ||
