...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.ANIMA_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service ANIMA_GATEWAY_TOKEN does not match gateway.auth.token in anima.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `anima gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
