...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "anima" | "animabot" | "animabot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["anima", "animabot", "animabot"] as const;
...
function isANIMAGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.anima.");
}
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("animabot") || lower.includes("animabot");
}
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "animabot" : "animabot",
        legacy: true,
      });
      continue;
    }
