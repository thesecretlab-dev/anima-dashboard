...
const TEMP_MEDIA_PREFIX = "anima-media-";
const tempMediaDirs: string[] = [];

async function createTempMediaDir() {
  const baseDir = resolvePreferredANIMATmpDir();
...
function createGroqAudioConfig(): ANIMAConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): ANIMAConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
