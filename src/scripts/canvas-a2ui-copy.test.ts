...
describe("canvas a2ui copy", () => {
  ...
  it("skips missing assets when ANIMA_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      const previous = process.env.ANIMA_A2UI_SKIP_MISSING;
      process.env.ANIMA_A2UI_SKIP_MISSING = "1";
      try {
        await expect(
        ...
        if (previous === undefined) {
          delete process.env.ANIMA_A2UI_SKIP_MISSING;
        } else {
          process.env.ANIMA_A2UI_SKIP_MISSING = previous;
        }
      }
    });
  });
