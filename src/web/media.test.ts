...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredANIMATmpDir(), "anima-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub ANIMA_STATE_DIR.
    // Also keep it outside the ANIMA temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["ANIMA_STATE_DIR"]);
    process.env.ANIMA_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "anima-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredANIMATmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("rejects default ANIMA state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
