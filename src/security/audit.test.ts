...
describe("security audit", () => {
  ...
  const withStateDir = async (label: string, fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ ANIMA_STATE_DIR: tmp }, async () => await fn(tmp));
  };

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "anima-security-audit-"));
  });
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("anima-sbx-browser-old\nanima-sbx-browser-missing-hash\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "anima-sbx-browser-old") {
        return {
      ...
      if (args[0] === "inspect" && args.at(-1) === "anima-sbx-browser-missing-hash") {
        return {
    ...
    expect(staleEpoch?.detail).toContain("anima-sbx-browser-old");
  });
  ...
  it("flags sandbox browser containers with non-loopback published ports", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("anima-sbx-browser-exposed\n"),
          stderr: Buffer.alloc(0),
          code: 0,
        };
      }
      if (args[0] === "inspect" && args.at(-1) === "anima-sbx-browser-exposed") {
        return {
      ...
      if (args[0] === "port" && args.at(-1) === "anima-sbx-browser-exposed") {
        return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-anima.json");
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withStateDir("discord-name-based-allowlist", async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.anima/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<ANIMAConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: ANIMAConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when install records drift from installed package versions", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@anima/voice-call", version: "9.9.9" }),
      "utf-8",
    );
    await fs.writeFile(
      path.join(hookDir, "package.json"),
      JSON.stringify({ name: "@anima/test-hooks", version: "8.8.8" }),
      "utf-8",
    );
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    beforeEach(() => {
      envSnapshot = captureEnv(["ANIMA_GATEWAY_TOKEN", "ANIMA_GATEWAY_PASSWORD"]);
      delete process.env.ANIMA_GATEWAY_TOKEN;
      delete process.env.ANIMA_GATEWAY_PASSWORD;
    });
