...
const RELAY_TOKEN_CONTEXT = "anima-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const ANIMA_RELAY_BROWSER = "ANIMA/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.ANIMA_GATEWAY_TOKEN?.trim() || process.env.ANIMABOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAuthTokenForPort(port: number): string {
  ...
  throw new Error(
    "extension relay requires gateway auth token (set gateway.auth.token or ANIMA_GATEWAY_TOKEN)",
  );
}

export async function probeAuthenticatedANIMARelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === ANIMA_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
