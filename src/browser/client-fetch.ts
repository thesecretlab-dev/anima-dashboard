...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the ANIMA gateway (ANIMA.app menubar, or \`${formatCliCommand("anima gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the ANIMA browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the ANIMA browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
