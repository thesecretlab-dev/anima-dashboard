  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          anima: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    process.env.ANIMA_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ANIMA_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ANIMA_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPassword === undefined) {
      ...
      process.env.ANIMA_GATEWAY_PASSWORD = prevGatewayPassword;
    }
