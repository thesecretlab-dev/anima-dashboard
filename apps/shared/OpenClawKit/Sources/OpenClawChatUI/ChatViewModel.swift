...
private let chatUILogger = Logger(subsystem: "ai.anima", category: "ANIMAChatUI")
...
public final class ANIMAChatViewModel {
    ...
    private static func stripInboundMetadata(from message: ANIMAChatMessage) -> ANIMAChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> ANIMAChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return ANIMAChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: ANIMAChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [ANIMAChatMessage],
        incoming: [ANIMAChatMessage]) -> [ANIMAChatMessage]
    {
